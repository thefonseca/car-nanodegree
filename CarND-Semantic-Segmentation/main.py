...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        for step, (images, gt) in enumerate(get_batches_fn(batch_size)):
            ...
            print('Epoch %d - step % d: loss = %.2f (%.3f sec)' % (epoch, step, loss_value, duration))
            
        if saver:
            save_path = saver.save(sess, os.path.join(ckp_dir, 'ckp'), global_step=epoch)
