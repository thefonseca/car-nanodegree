...
def LeNet(x):
    ...
    conv1 = tf.nn.bias_add(tf.nn.conv2d(x, filter1, [1, 1, 1, 1], 'VALID'), b1)
    conv1 = tf.nn.relu(conv1)
    conv1 = tf.nn.max_pool(conv1, [1, 2, 2, 1], [1, 2, 2, 1], 'VALID')

    filter2 = tf.Variable(tf.truncated_normal([5, 5, 6, 16]))
    b2 = tf.Variable(tf.zeros(16))
    conv2 = tf.nn.bias_add(tf.nn.conv2d(conv1, filter2, [1, 1, 1, 1], 'VALID'), b2)
    conv2 = tf.nn.relu(conv2)
    conv2 = tf.nn.max_pool(conv2, [1, 2, 2, 1], [1, 2, 2, 1], 'VALID')
    
    flat = tf.contrib.layers.flatten(conv2)
    W1 = tf.Variable(tf.truncated_normal([400, 120]))
    b3 = tf.Variable(tf.zeros(120))
    fc1 = tf.matmul(flat, W1) + b3
    fc1 = tf.nn.relu(fc1)
    
    W2 = tf.Variable(tf.truncated_normal([120, 10]))
    b4 = tf.Variable(tf.zeros(10))
    return tf.matmul(fc1, W2) + b4
