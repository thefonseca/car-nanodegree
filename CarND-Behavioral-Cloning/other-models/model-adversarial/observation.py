...
def sensorimotor_generator(generator, frames_per_concept=5, 
                           batch_size=32, scale_factor=None,
    ...
    while 1: # Loop forever so the generator never terminates
        ...
        for offset_batch in range(0, num_samples - (batch_size + frames_per_concept + 1), 
                                  batch_size + frames_per_concept + 1):
            ...
            for offset in range(offset_batch, offset_batch + batch_size):
                concept_samples = samples[offset:offset+frames_per_concept+1]
                ...
                if grayscale:
                    images_tensor = images_tensor.reshape((1,) + images_tensor.shape[1:] + (frames_per_concept+1,))
                else:
                    images_tensor = images_tensor.reshape((1,) + images_tensor.shape[1:] + (images_tensor.shape[-1] * frames_per_concept+1,))

                actions_tensor = np.array(angles)
                actions_tensor = actions_tensor.reshape((batch_size, frames_per_concept+1) + (1,))

                #concept = concept_model.predict([images_tensor[:,:-1], actions_tensor[:,:-1]], verbose=0)
                image_prediction, action_prediction = generator.predict(
                    [images_tensor[:,:,:,:-1], actions_tensor[:,:-1]], verbose=0)
                ...
                print(real_images_seq.shape, image_prediction.shape)
